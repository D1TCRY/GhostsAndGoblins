...
# STATE
from ...state import Sprite, Direction
...
class Platform(Actor):
    ...
    def move(self, arena: Arena) -> None:
        """Le piattaforme sono statiche."""

        return

    def sprite(self) -> Sprite | None:
        """Di default non è associato alcuno sprite,
        il comportamento delle Platform è principalmente logico."""
    ...
    def check_collision(self, obj: Actor) -> bool:
        """Verifica se un attore collide con la piattaforma."""
    ...
    def clamp(self, obj: Actor) -> tuple[Direction | None, float, float]:
        """Calcola la traslazione minima per separare un attore dalla piattaforma.

        Se non ce sovrapposizione, restituisce (None, 0.0, 0.0).

        In caso di collisione:
        - determina se la intersezione è prevalente in orizzontale o verticale
        - calcola la direzione di contatto (LEFT, RIGHT, UP, DOWN)
        - restituisce una coppia di offset (dx, dy) da applicare a obj
          per portarlo appena fuori dalla piattaforma

        Se la direzione calcolata non è presente in contact_surfaces,
        restituisce (None, 0.0, 0.0).
        """

        # -> coordinate e dimensioni dell'attore
        ox, oy = obj.pos()
        ow, oh = obj.size()
        ...
        # -> estremi della piattaforma
        p_left = self.x
        p_top = self.y
        p_right = self.x + self.width
        p_bottom = self.y + self.height

        # -> se non cè sovrapposizione tra attore e piattaforma, ritorna nessuna collisione
        if o_right <= p_left or o_left >= p_right or o_bottom <= p_top or o_top >= p_bottom:
            return None, 0.0, 0.0

        # -> calcolo delle dimensioni dell'overlap in x e y e dei centri di attore e piattaforma
