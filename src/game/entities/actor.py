...
class Arena:
    ...
    def __init__(self, size: Point):
        ...
        self._turn = -1
        self._actors = []
        self._curr_keys = self._prev_keys = list()
        self._collisions = []
    ...
    def tick(self, keys=None):
        ...
        self._curr_keys = keys if keys is not None else []
    ...
    # ======== NEW METHODS ========
    def set_sprite(self, value: Sprite) -> None:
        if not isinstance(value, Sprite):
            raise TypeError("sprite must be of type Sprite")
        self._arena_sprite = value

    def sprite(self) -> Sprite | None:
        if hasattr(self, '_arena_sprite'):
            return self._arena_sprite
        else:
            return None